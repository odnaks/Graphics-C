...
void	drawfr(t_param *param)
{
	...
	while (y < param->h)
	{
		...
		while (x < param->w)
		{
			newRe = 1.5 * (x - param->w / 2) / (0.5 * param->zoom * param->w) + param->move_x;
			newIm = (y - param->h / 2) / (0.5 * param->zoom * param->h) + param->move_y;
			...
			while (i < maxIterations)
			{
				oldRe = newRe;
				oldIm = newIm;
				newRe = oldRe * oldRe - oldIm * oldIm + param->cRe_s;
				newIm = 2 * oldRe * oldIm + param->cIm_s;
